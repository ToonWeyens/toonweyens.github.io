...
Nov '25 - Nov '25
:   **Online Course - Fine-tuning & RL for LLMs: Intro to Post-training** - [DeepLearning.AI](https://learn.deeplearning.ai/courses/fine-tuning-and-reinforcement-learning-for-llms-intro-to-post-training)

    Course aimed at transforming raw LLMs into production-ready applications using post-training techniques like SFT and RLHF. Taught by Sharon Zhou.

    - **Alignment**: Use Supervised Fine-Tuning (SFT) and Reinforcement Learning (RLHF) to shape model behavior, improve reasoning, and ensure safety.

    - **Algorithms**: Implement advanced optimization techniques like PPO and GRPO (Group Relative Policy Optimization) to guide models toward desired outcomes.

    - **Evaluation**: Design rigorous testing frameworks to detect "reward hacking" and diagnose failures, ensuring models are reliable before deployment.

    - **Efficiency**: Apply Low-Rank Adaptation (LoRA) to fine-tune massive models on standard hardware without the heavy computational cost of full training.

    - **Ops**: Build production pipelines that monitor model performance and use real-world logs to create data feedback loops for continuous improvement.
